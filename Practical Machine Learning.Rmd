...
set.seed(3433)
```
After this, I started with some preliminary data exploration. Looking at the dataset, I saw the first column was displaying the record number, so I dropped this column from my data set.
...
```
I split my dataset in a training set and a validation set.
```{r}
...
str(training)
```
I noticed there were a lot of variables with a large amount of empty values and/or NA. I tried to get rid of these by removing the columns with near zero variance.
```{r}
...
### Modeling
My first try was to use a Classification tree. I decided not to center and scale the variables, since this is not really useful for this method.
```{r}
